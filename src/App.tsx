...
async function syncToFirestore(data: AppData) {
  const batch = writeBatch(db)

  for (const worker of data.workers) {
    // Worker doc
    batch.set(doc(db, 'workers', worker.id), {
      id: worker.id,
      name: worker.name,
      role: worker.role,
    ...
    // Entries
    for (const [dateKey, entry] of Object.entries(worker.entries)) {
      const entryId = `${worker.id}_${dateKey}`
      batch.set(doc(db, 'entries', entryId), {
        worker_id: worker.id,
        date: dateKey,
    ...
    // Monthly cities
    for (const [monthKey, text] of Object.entries(worker.monthlyCities)) {
      const cityId = `${worker.id}_${monthKey}`
      batch.set(doc(db, 'monthly_cities', cityId), {
        worker_id: worker.id,
        month_key: monthKey,
        cities_text: text,
      })
...
function calculateMonthlyPayroll(worker: Worker, year: number, month: number): MonthlyPayroll {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (!weeksMap.has(weekKey)) {
      weeksMap.set(weekKey, { weekdayDaysInMonth: 0, weekdayHolidays: 0, weekdayHours: 0, festiveHours: 0 })
    }
    const bucket = weeksMap.get(weekKey)!
    const hours = numberOrZero(entry.workedHours)

    totalWorkedHours += hours

    if (isWeekend(date)) {
      bucket.festiveHours += hours
    } else {
      bucket.weekdayDaysInMonth += 1
      if (entry.isHoliday) {
        bucket.weekdayHolidays += 1
        bucket.festiveHours += hours
      } else {
        bucket.weekdayHours += hours
      }
    }
  }

  weeksMap.forEach((bucket) => {
    const weeklyThreshold = (bucket.weekdayDaysInMonth - bucket.weekdayHolidays) * 8
    totalOvertimeNormalHours += Math.max(0, bucket.weekdayHours - weeklyThreshold)
    totalOvertimeFestiveHours += bucket.festiveHours
  })
...
export default function App() {
  ...
  async function loadFromFirestore() {
    try {
      ...
      const [workersSnap, entriesSnap, citiesSnap] = await Promise.all([
        getDocs(collection(db, 'workers')),
        getDocs(collection(db, 'entries')),
        getDocs(collection(db, 'monthly_cities')),
      ])

      const workers = workersSnap.docs.map(d => d.data())
      const entries = entriesSnap.docs.map(d => d.data())
      const cities = citiesSnap.docs.map(d => d.data())
  ...
  async function updateWorker(workerId: string, updater: (worker: Worker) => Worker) {
    ...
    try {
      await setDoc(doc(db, 'workers', updated.id), {
        id: updated.id,
        name: updated.name,
        role: updated.role,
  ...
  function updateDay(workerId: string, dateKey: string, entry: DayEntry, force: boolean = false) {
    ...
    const performSync = async () => {
      ...
      try {
        const entryId = `${workerId}_${dateKey}`
        ...
        console.error('Error actualizando día en Firebase:', err)
      }
  ...
  async function updateMonthlyCities(workerId: string, text: string) {
    ...
    try {
      const cityId = `${workerId}_${monthKey}`
      await setDoc(doc(db, 'monthly_cities', cityId), {
        worker_id: workerId,
        month_key: monthKey,
        cities_text: text,
      })
    } catch (err) {
      console.error('Error actualizando ciudades:', err)
    }
  }
  ...
  async function removeWorker(workerId: string) {
    ...
    try {
      ...
      const entriesSnap = await getDocs(
        query(collection(db, 'entries'), where('worker_id', '==', workerId))
      )
      const batch = writeBatch(db)
      entriesSnap.docs.forEach(d => batch.delete(d.ref))

      // Delete associated monthly_cities
      const citiesSnap = await getDocs(
        query(collection(db, 'monthly_cities'), where('worker_id', '==', workerId))
      )
      citiesSnap.docs.forEach(d => batch.delete(d.ref))
    ...
    reader.onload = async () => {
      try {
        ...
        if (parsed?.workers) {
          ...
          // Sync restored data to Firebase immediately after import
          setLoading(true)
          await syncToFirestore(parsed)
          setLoading(false)
          ...
          {selectedWorker && selectedMonthly && selectedAnnual && (
            <div className="bottom-grid">
              ...
              <section className="card detail-card">
                ...
                <p className="muted">
                  Se contabilizan <strong>8 horas diarias</strong> como jornada ordinaria. El umbral semanal se calcula en base a los días laborables (L-V) del mes (ej: <strong>40 h</strong> para semana completa, o proporcional si el mes corta la semana).
                  Los festivos o días marcados reducen el umbral.
                  Ejemplo: semana de 4 días en el mes con 9+8+8+8 horas trabajadas → 33 h − 32 h (4 días × 8 h) = <strong>1 h extra</strong>.
                </p>
